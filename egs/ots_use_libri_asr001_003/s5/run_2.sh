...
#data=/data/LibriSpeech-asr-corpus-SLR12
...
#local/g2p/train_g2p.sh data/local/dict/cmudict data/local/lm
  ...
  #local/prepare_dict.sh --stage 3 --nj 32 --cmd "$train_cmd" data/local/lm data/local/lm data/local/dict_nosp
  #exit 0
...
if [ $stage -le 6 ]; then
    for part in dev_clean_libri test_clean_libri dev_other_libri test_other_libri dev_asr001 test_asr001 dev_asr003 test_asr003 train_libri_960_use_asr001_asr003; do
...
if [ $stage -le 8 ]; then
  # train a monophone system on train clean 100 hours
  steps/train_mono.sh --boost-silence 1.25 --nj 32 --cmd "$train_cmd" data/train_clean_100 data/lang_nosp exp/mono
...
if [ $stage -le 9 ]; then
  ...
  # train a first delta + delta-delta triphone system on train clean 100 hours
  steps/train_deltas.sh --boost-silence 1.25 --cmd "$train_cmd" \
                        2000 10000 data/train_clean_100 data/lang_nosp exp/mono_ali exp/tri1
  ...
  (
    ...
    for test in test_clean test_other dev_clean dev_other; do
      steps/decode.sh --nj 32 --cmd "$decode_cmd" exp/tri1/graph_nosp_tgsmall \
...
if [ $stage -le 10 ]; then
  ...
  steps/train_lda_mllt.sh --cmd "$train_cmd" \
                          ...
                          data/train_clean_100 data/lang_nosp exp/tri1_ali exp/tri2b
  ...
  (
    ...
    for test in test_clean test_other dev_clean dev_other; do
      steps/decode.sh --nj 32 --cmd "$decode_cmd" exp/tri2b/graph_nosp_tgsmall \
...
if [ $stage -le 11 ]; then
  # Align train clean 100 hours using the tri2b model
  steps/align_si.sh  --nj 32 --cmd "$train_cmd" --use-graphs true \
                     data/train_clean_100 data/lang_nosp exp/tri2b exp/tri2b_ali

  # Train tri3b, which is LDA+MLLT+SAT on train clean 100 hours 
  steps/train_sat.sh --cmd "$train_cmd" 2500 15000 \
                     data/train_clean_100 data/lang_nosp exp/tri2b_ali exp/tri3b
